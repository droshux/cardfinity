...
import Editor.Conditions (asEffect, conditionEditor)
...
destroy =
  let get (DestroyM b fc) = flip A.DestroyEnemy fc $ if b then A.Banish else A.Discard
      set m = \case
...
optionalEffect =
  let set _ (A.Optional e) = setEffect e
      set ie _ = ie
...
attackEditor = M.component False (const $ M.modify not) view
  where
    view = const $ H.button_ [H.onClick ()] [text "Toggle"]
...
search =
  let get (d, t) = (A.Search . (if d then A.DrillFor else A.SearchFor)) t
      set = const $ \case
