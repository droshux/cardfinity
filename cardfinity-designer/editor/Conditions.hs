...
{-# LANGUAGE LambdaCase #-}
module Conditions (condition, conditionEditor,asEffect ) where
...
destroy  = let
    get (DestroyM b fc) = flip A.Destroy fc $ if b then A.Banish else A.Discard
    set = const $ \case
...
youMay = let
    set _ (A.YouMay c) = setCondition c
    set ic _ = ic
